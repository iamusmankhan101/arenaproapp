...
export default function AddVenueModal({ open, onClose, editVenue = null, vendorId = null }) {
  ...
  const handlePeakHoursChange = (field) => (event) => {
    setFormData({
      ...formData,
      peakHours: {
        ...formData.peakHours,
      ...
      case 2:
        return (
          <Grid container spacing={2} sx={{ mt: 1 }}>
            ...
            {/* Peak Hours Configuration */}
            <Grid item xs={12}>
              <Typography variant="h6" sx={{ color: '#004d43', fontWeight: 600, mt: 3, mb: 2 }}>
                Peak Hours Configuration
              </Typography>
              <Card variant="outlined" sx={{ p: 2, bgcolor: '#f0f7f6', borderColor: '#004d43' }}>
                <Box sx={{ display: 'flex', alignItems: 'center', gap: 2, mb: 2 }}>
                  <input
                    type="checkbox"
                    checked={formData.peakHours.enabled}
                    onChange={handlePeakHoursChange('enabled')}
                    disabled={loading}
                  ...
                  <Grid container spacing={2}>
                    <Grid item xs={12} md={4}>
                      <TextField
                        ...
                        type="time"
                        value={formData.peakHours.startTime}
                        onChange={handlePeakHoursChange('startTime')}
                        disabled={loading}
                        InputLabelProps={{ shrink: true }}
                        helperText="e.g., 5:00 PM"
                      />
                    </Grid>
                    <Grid item xs={12} md={4}>
                      <TextField
                        ...
                        type="time"
                        value={formData.peakHours.endTime}
                        onChange={handlePeakHoursChange('endTime')}
                        disabled={loading}
                        InputLabelProps={{ shrink: true }}
                        helperText="e.g., 11:00 PM"
                      />
                    </Grid>
                    <Grid item xs={12} md={4}>
                      <TextField
                        ...
                        type="number"
                        value={formData.peakHours.priceMultiplier}
                        onChange={handlePeakHoursChange('priceMultiplier')}
                        disabled={loading}
                        inputProps={{ min: 1, step: 0.1 }}
                        helperText="e.g., 1.5 = 50% markup"
                      />
                    </Grid>
                    <Grid item xs={12}>
                      <Typography variant="caption" color="text.secondary">
                        Peak price will be: PKR {(parseFloat(formData.basePrice) * parseFloat(formData.peakHours.priceMultiplier)).toFixed(0)} per slot
                      </Typography>
                    </Grid>
                  </Grid>
                )}
