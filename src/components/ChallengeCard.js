...
export default function ChallengeCard({ challenge, onAccept, onViewDetails, onDelete, userTeam, currentUserId }) {
  ...
  const isOwnChallenge = React.useMemo(() => {
    ...
    // Get all possible creator IDs
    const creatorId = actualChallenge.creatorTeam?.id || actualChallenge.challengerId || '';
    const normalizedCreatorId = String(creatorId).toLowerCase().trim();
    const normalizedUserId = String(currentUserId || '').toLowerCase().trim();
    const normalizedTeamId = String(userTeam?.id || '').toLowerCase().trim();

    // Check if any of the IDs match
    const isOwn = (normalizedUserId && normalizedCreatorId && normalizedCreatorId === normalizedUserId) ||
      (normalizedTeamId && normalizedCreatorId && normalizedCreatorId === normalizedTeamId);

    console.log('🔍 ChallengeCard isOwnChallenge check:', {
      challengeId: actualChallenge.id,
      challengeName: actualChallenge.title,
      creatorId,
      normalizedCreatorId,
      currentUserId,
      normalizedUserId,
      ...
      normalizedTeamId,
      isOwn,
      match1: normalizedUserId && normalizedCreatorId && normalizedCreatorId === normalizedUserId,
      match2: normalizedTeamId && normalizedCreatorId && normalizedCreatorId === normalizedTeamId
    });
  ...
  const hasUserJoined = React.useMemo(() => {
    if (!actualChallenge || !currentUserId) return false;
    const participants = actualChallenge.participants || [];
    return participants.some(p => String(p.id).toLowerCase().trim() === String(currentUserId).toLowerCase().trim());
  }, [actualChallenge, currentUserId]);

  // Calculate available spots based on format
  const calculateSpots = () => {
    // First check if custom player counts are specified (takes priority)
    const hasCustomCounts = actualChallenge.currentPlayers || actualChallenge.needPlayers;
    
    if (hasCustomCounts) {
      const currentPlayers = parseInt(actualChallenge.currentPlayers) || 0;
      const neededPlayers = parseInt(actualChallenge.needPlayers) || 0;
      
      if (neededPlayers > 0) {
        return { total: currentPlayers + neededPlayers, creatorCount: currentPlayers, needed: neededPlayers };
      }
    }
    
    // For Padel with format but no custom counts, use format defaults
    if (actualChallenge.sport === 'Padel' && actualChallenge.format) {
      if (actualChallenge.format === '1v1') {
        // 1v1 = 2 total players (1 creator + 1 opponent)
        return { total: 2, creatorCount: 1, needed: 1 };
      } else if (actualChallenge.format === '2v2') {
        // 2v2 = 4 total players (2 on creator's side + 2 opponents)
        return { total: 4, creatorCount: 2, needed: 2 };
      }
    }
    
    // Default fallback: 1v1 (need 1 opponent)
    return { total: 2, creatorCount: 1, needed: 1 };
  };

  const spots = calculateSpots();
  const participants = actualChallenge.participants || [];
  // Filled spots = creator's team size + participants who joined
  const filledSpots = spots.creatorCount + participants.length;
  const availableSpots = spots.needed - participants.length; // spots needed minus those who joined
  const hasAvailableSpots = availableSpots > 0;

  const isAccepted = actualChallenge.status === 'accepted';
  const canAccept = !isOwnChallenge && !hasUserJoined && hasAvailableSpots && actualChallenge.status === 'open';
      ...
      <Card style={styles.card}>
        ...
        <Card.Content style={styles.content}>
          ...
          <View style={styles.teamsContainer}>
            ...
            <View style={styles.teamBlock}>
              <View style={{ flexDirection: 'row', justifyContent: 'center', marginBottom: 8 }}>
                {(() => {
                  ...
                  if (actualChallenge.sport === 'Padel') {
                    if (actualChallenge.format === '1v1') count = 1;
                    else if (actualChallenge.format === '2v2') count = 2;
                  }
              ...
              <Text numberOfLines={1} style={styles.teamName}>
                {String(actualChallenge.teamName || 'Team')}
                {actualChallenge.creatorTeam?.isIndividual && (
                  <Text style={{ fontSize: 9, color: '#999' }}> (Solo)</Text>
                )}
              </Text>
            ...
            <View style={styles.teamBlock}>
                    ...
                    <View style={{ flexDirection: 'row', justifyContent: 'center', marginBottom: 6 }}>
                      {actualChallenge.participants.slice(0, 3).map((p, i) => (
                        <View key={i} style={{
                          ...
                          {p.avatar || p.photoURL ? (
                            <Avatar.Image size={32} source={{ uri: p.avatar || p.photoURL }} />
                          ) : (
              ...
              ) : actualChallenge.creatorTeam?.isIndividual && actualChallenge.participants && actualChallenge.participants.length > 0 ? (
                // Individual challenge with participants
                <>
                  <View style={{ flexDirection: 'row', justifyContent: 'center', marginBottom: 8 }}>
                    {actualChallenge.participants.slice(0, 3).map((participant, i) => (
                      <View key={participant.id || i} style={{
                        marginLeft: i === 0 ? 0 : -12,
                        zIndex: 3 - i,
                        borderRadius: 22,
                        borderWidth: 2,
                  ...
                  <Text numberOfLines={1} style={styles.teamName}>
                    {actualChallenge.participants.length === 1 
                      ? String(actualChallenge.participants[0].name || 'Player')
                      : String(actualChallenge.participants.length + ' Players')}
                  </Text>
                  <View style={styles.statBadge}>
                    <MaterialIcons name="people" size={12} color="#fbc02d" />
                    <Text style={styles.statText}>{String(actualChallenge.participants.length + ' Joined')}</Text>
                  </View>
                </>
          ...
          <View style={styles.detailsContainer}>
            ...
            {/* Row 5: Available Spots */}
            {(actualChallenge.currentPlayers || actualChallenge.needPlayers || actualChallenge.format) && (
              <View style={[styles.detailRow, { marginTop: 4 }]}>
                <MaterialIcons name="people" size={16} color="#004d43" style={styles.detailIcon} />
                <Text style={styles.detailText}>
                  {`${filledSpots}/${spots.total} Players`}
                  {availableSpots > 0 && (
