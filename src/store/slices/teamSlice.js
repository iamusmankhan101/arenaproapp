...
export const acceptChallenge = createAsyncThunk(
  ...
  async ({ challengeId, opponentId, opponentName, opponentData, acceptAsTeam, teamData }, { rejectWithValue }) => {
    try {
      const result = await challengeService.acceptChallenge(challengeId, opponentId, opponentName, opponentData, acceptAsTeam, teamData);
      if (result.success) {
        return { 
          id: challengeId, 
          status: 'accepted', 
          ...
          ...(acceptAsTeam && teamData && {
            acceptedTeam: teamData
          })
        };
      } else {
        return rejectWithValue(result.error);
